...
Проект состоит из основной части (ядро), двух библиотек (kav_fileOperations, kav_logger) и монитора (kav_logger_monitor). Все части подробно описаны в документации.\
...
- три независимых подпроекта:
  - kav_fileOperations - библиотека для работы с файлами;
...
- libraries/nlohmann_json - библиотека для работы с JSON-форматом;
- libraries/kav_fileOperations - библиотека для работы с файлами;
- libraries/kav_logger - библиотека для логирования событий в файл и вывода на консоль;
...
8. Подключить библиотеку kav_fileOperations (в проекте это уже сделано).
...
Внимание!!! В CMakeLists.txt используется библиотека lwinpthread как статическая, в следующей строке:
Установить языковые флаги при сборке для всех конфигураций. Эти флаги будут передаваться при всех вызовах компилятора\
set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -static-libstdc++ -static-libgcc -static -LC:\\MinGW-W64\\x86_64-13.2.0-release-posix-seh-ucrt-rt_v11-rev1\\x86_64-w64-mingw32\\lib -lwinpthread")\
То есть, надо будет самостоятельно прописать путь до библиотеки.\
...
Также можно дополнительно использовать ключи для запуска программы с параметрами. Подробное описание аргументов приводится в [документации](./docs/ru/index.md).
...
В проекте использована система юнит-тестирования [GoogleTest](https://github.com/google/googletest), - в репозитории в директории "tests" имеются исходные тесты. Протестирована обязательная проверка на недоступность файлов конфигурации и файлов документов, заявленная функциональность, а также наиболее сложные функции библиотек и основной программы. Подробно о функциях тестирования можно ознакомиться в [документации](./docs/ru/index.md). 
