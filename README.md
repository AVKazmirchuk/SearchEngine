...
Проект состоит из основной части (ядро), двух библиотек (kav_check, kav_logger) и монитора. Все части подробно описаны в документации.\
...
- три независимых подпроекта:
  ...
  - logger_monitor - консоль вывода сообщений работы основной программы.
...
- monitor - файлы подпроекта вывода сообщений на консоль;
...
2. Скопировать файл logger_monitor.exe из директории подпроекта "logger-monitor" в директорию с исполняемым файлом основного проекта search-engine.exe.
...
Для работы необходимо, чтобы файлы конфигурации и запросов (config.json, logger.json и requests.json) находились в директории с программой. В этой же директории по окончании поиска будет создан файл ответов answers.json.
