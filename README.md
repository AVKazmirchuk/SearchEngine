...
- скомпилировать из своего проекта (основной проект - search_engine, подпроект - search_engine_monitor). Обратите внимание, что необходимо так же скопировать файлы config.json, logger.json, requests.json (конфигурации и запросов) из репозитория директории "resources", и файл search_engine_monitor.exe из директории подпроекта "search_engine_monitor", в директорию с исполняемым файлом search-engine.exe.\
